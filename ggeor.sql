...
job_id INT(4) NOT NULL,
interview_id INT(4) NOT NULL,
interview_date DATE NOT NULL,
start_time FLOAT(4,2),
duration FLOAT(4,2),
comments TEXT,
personality_score INT(10),
education_score INT(10),
experience_score INT(10),
candidate_name VARCHAR(12),
recruiter_name VARCHAR(12),
PRIMARY KEY (interview_id),
CONSTRAINT RECR_NAME FOREIGN KEY (recruiter_name) REFERENCES `recruiter`(username) ON DELETE CASCADE ON UPDATE CASCADE,
CONSTRAINT ID FOREIGN KEY (job_id) REFERENCES `applies`(job_id) ON DELETE CASCADE ON UPDATE CASCADE,
CONSTRAINT CAND_NAME FOREIGN KEY (candidate_name) REFERENCES `candidate`(username) ON DELETE CASCADE ON UPDATE CASCADE
)engine=InnoDB;

insert into interviews(job_id,interview_id,interview_date,start_time,duration,comments,personality_score,education_score,experience_score,candidate_name,recruiter_name) values(2,2,'2018-03-04',15.30,02.15,'poly kalos',4,4,4,'cleogeo','msmith'), (7,3,'2017-06-13',14.30,01.30,'kalos',3,3,4,'zazahir23','n_tri'), (6,4,'2019-02-25',12.15,45.00,'para poly kalos',5,5,3,'lionarF','papad'), (12,6,'2014-10-01',11.10,50.00,'kakos',2,2,1,'mnikol','bettyg'), (9,7,'2016-06-15',20.20,01.15,'teleios',5,5,5,'abrown','varcon82');
...
insert into tomeis(title,keimeno) 
values('cars','ybridika aytokinita'),
('computers','kataskeyi logismikoy'),
('restaurants','biologiko fagito');

CREATE TABLE tomeis_etaireias(
company CHAR(9),
section VARCHAR(20),
CONSTRAINT comp FOREIGN KEY ( company ) REFERENCES `etaireia`( AFM ) ON DELETE CASCADE ON UPDATE CASCADE,
CONSTRAINT sec FOREIGN KEY ( section ) REFERENCES `tomeis` ( title ) ON DELETE CASCADE ON UPDATE CASCADE
);

insert into tomeis_etaireias(company,section) values('023453344','cars'),
('23122345','computers'),
('561234561','restaurants');

CREATE TABLE ACTIVITIES(
username VARCHAR(12),
activity_id INT(4) NOT NULL,
activity_time FLOAT(4,2) NOT NULL,
activity_date DATE NOT NULL,
epityxia ENUM('Nai','Oxi'),
activity_type ENUM('insert','update','delete'),
onoma_pinaka VARCHAR(20),
PRIMARY KEY(activity_id),
CONSTRAINT username FOREIGN KEY( username ) REFERENCES `candidate`( username ) ON DELETE CASCADE ON UPDATE CASCADE,
 FOREIGN KEY( username ) REFERENCES `recruiter`( username ) ON DELETE CASCADE ON UPDATE CASCADE
);


 
b)SELECT username,certificates,count(*),AVG(grade)
 FROM has_degree
 INNER JOIN candidate ON has_degree.cand_usrname=candidate.username GROUP BY candidate.username HAVING count(*)>1;


c) SELECT cand_usrname, AVG(salary),count(*) 
FROM applies INNER JOIN job ON applies.job_id=job.id 
GROUP BY cand_usrname  
HAVING AVG(salary)>=1800;

d)SELECT name,edra,antikeim_title,position FROM etaireia
,job,requires
WHERE job.id=requires.job_id AND antikeim_title LIKE '%program%' AND edra LIKE '%Patra%';

a)  SELECT job.salary,recruiter.username, etaireia.name,job.id, count(*) FROM etaireia,recruiter,job,applies WHERE job.recruiter=recruiter AND applies.job_id=job.id AND recruiter.firm=etaireia.afm GROUP BY job.salary,recruiter.username,etaireia.name,job.id  HAVING job.salary>=1900;

e) SELECT username,count(*) counter_interviews ,count(1) counter_jobs,AVG(salary),announce_date FROM recruiter INNER JOIN interviews ON recruiter.username=interviews.recruiter_name INNER JOIN job ON recruiter.username=job.recruiter  GROUP BY username  HAVING job.announce_date>=2 ORDER BY 4 DESC;
